...
func ProveSet(x int64, r *big.Int, p ParamsSet) (ProofSet, error) {
	...
	proof_out.Cc, _ = HashSet(proof_out.A, proof_out.D)
	proof_out.Cc = Mod(proof_out.Cc, bn256.Order)

	proof_out.Zr = Sub(proof_out.M, Multiply(r, proof_out.Cc))
	proof_out.Zr = Mod(proof_out.Zr, bn256.Order)
	proof_out.Zsig = Sub(proof_out.s, Multiply(new(big.Int).SetInt64(x), proof_out.Cc))
	proof_out.Zsig = Mod(proof_out.Zsig, bn256.Order)
	proof_out.Zv = Sub(proof_out.t, Multiply(v, proof_out.Cc))
...
func ProveUL(x, r *big.Int, p paramsUL) (proofUL, error) {
	...
	// Fiat-Shamir heuristic
	proof_out.Cc, _ = Hash(proof_out.A, proof_out.D)
	proof_out.Cc = Mod(proof_out.Cc, bn256.Order)

	proof_out.Zr = Sub(proof_out.M, Multiply(r, proof_out.Cc))
	proof_out.Zr = Mod(proof_out.Zr, bn256.Order)
	for i = 0; i < p.l; i++ {
		proof_out.Zsig[i] = Sub(proof_out.s[i], Multiply(new(big.Int).SetInt64(decx[i]), proof_out.Cc))
		proof_out.Zsig[i] = Mod(proof_out.Zsig[i], bn256.Order)
		proof_out.Zv[i] = Sub(proof_out.t[i], Multiply(v[i], proof_out.Cc))
...
func VerifySet(proof_out *ProofSet, p *ParamsSet) (bool, error) {
	...
	// D == C^c.h^ zr.g^zsig ?
	D = new(bn256.G2).ScalarMult(proof_out.C, proof_out.Cc)
